...
export function useCanvasFlowChanges(
  ...
  const handleNodesChange = useCallback((changes: NodeChange[]): void => {
    ...
    const keyboardMoves = Object.keys(dragStartPositionsRef.current).length === 0
      ...
      : []
    if (keyboardMoves.length > 0) useStore.getState().moveNodes(keyboardMoves, Object.fromEntries(nodes.map(node => [node.id, { ...node.position }])))
    ...
    if (changesToApply.length > 0) applyFlowChanges(applyNodeChanges(changesToApply, nodes) as never)
  }, [applyFlowChanges, dragStartPositionsRef, nodes])
